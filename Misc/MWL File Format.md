...
This format should be accurate as of Lunar Magic v3.63.
...
The level information contains various settings bytes for the level:

- Bytes 0-1 (2 bytes) contain the source level number.
- Bytes 2-8 (7 bytes) contain the first five bytes of the secondary level header. The last two bytes appear to be unused for now.
- Bytes 9-13 (5 bytes) contain the four bytes of the midway point's data tables. The fifth byte appears to be unused for now.
- Bytes 14-17 (4 bytes) contain additional bytes from the secondary level header, ordered $06FC00, $06FE00, ``read3(read3($05D9A2)+70)``, and $06FA00 respectively.
...
Palette data is preceded by an 8-byte header. Bytes 4-6 of this are the source address of the data from the original ROM (or 000000 if a custom palette is not in use), but do not seem to actually be used for anything.
...
The data starts with an 8-byte header:

- Byte 0 of this contains the value for $03FE00.
- Bytes 4-6 contain the source address of the data from the original ROM (or 000000 if absent), but do not seem to actually be used for anything.
- The remaining bytes appear to be unused.
...
This data has no header and consists solely of sixteen 16-bit values, with one ExGFX file per value.
